...
session_start();
$biodataSiswa = mysqli_query($con,"SELECT * FROM siswa JOIN kelas ON siswa.id_kelas=kelas.id_kelas WHERE nisn='".$_GET['nisn']."'");
$historyPembayaran = mysqli_query($con, "SELECT * FROM pembayaran JOIN petugas ON pembayaran.id_petugas=petugas.id_petugas JOIN spp ON pembayaran.id_spp=spp.id_spp WHERE nisn='".$_GET['nisn']."' ORDER BY tgl_bayar");
